...
fn synced_change_id_after_remote_apply(
    ...
    pre_apply_local_change_id: Option<i64>,
    post_apply_change_id: i64,
) -> i64 {
    if replayed_local_changes {
        pre_apply_local_change_id.unwrap_or(0)
    } else {
...
mod tests {
    use super::{
        ...
        use_pushed_replay_floor_hint_for_local_replay, DatabaseSyncEngine, DatabaseSyncEngineOpts,
    ...
    fn remote_apply_acknowledges_pre_apply_cdc_when_local_changes_are_recaptured() {
        ...
        assert_eq!(
            ...
            12,
            "with local replay, only pre-apply CDC is acknowledged so recaptured local rows remain pushable"
        );
        assert_eq!(
    ...
    fn replace_base_recaptures_replayed_local_changes_for_push() {
        ...
        let mut gen = genawaiter::sync::Gen::new({
            ...
            move |coro| async move {
                ...
                let pre_apply_change_id = max_local_change_id(&coro, &conn).await?.unwrap_or(0);
                drop(conn);
                ...
                assert_eq!(
                    ...
                    Some(pre_apply_change_id),
                    "remote apply must acknowledge CDC through the pre-apply high-water while leaving recaptured local rows pushable"
                );
